import {
    ...
    PlayerId, ResolutionContext, TargetType,
    Zone
...
export class MoveEffectHandler {
  public static handle(
    ...
    if (!effect.zone) {
      if (
        ...
        (effect.type as any) === "ExileTopCard" ||
        (effect.type as any) === "ExileAllCards"
      ) {
        effect.zone = Zone.Exile;
      } else if (
        effect.type === EffectType.DrawCards ||
        (effect.type as any) === "ReturnToHand" ||
        effect.type === EffectType.MoveToZone ||
        (effect.type as any) === "PutInHand"
      ) {
        effect.zone = Zone.Hand;
      } else if (
        effect.type === EffectType.DiscardCards ||
        (effect.type as any) === "Mill" ||
    ...
    if (effect.type === "DrawCards") {
      return this.resolveDrawCards(state, effect, log, context, finalTargetIds);
    }
    if (effect.type === "Mill") {
      return this.resolveMill(state, effect, log, context, finalTargetIds);
    }
    if (effect.type === "SearchLibrary") {
      return this.resolveLibrarySearch(
        state,
        {
          ...effect,
    ...
    if (effect.type === "Scry") {
      return this.resolveScry(state, effect, log, context, finalTargetIds);
    }
    if (effect.type === "Surveil") {
      return this.resolveSurveil(state, effect, log, context, finalTargetIds);
    }
    if (effect.type === EffectType.LookAtTopAndPick) {
      return this.resolveLookAtTopAndPick(
        state,
        effect,
        log,
        context,
    ...
    if (effect.type === EffectType.ExchangeHandAndGraveyard) {
      return this.resolveExchangeHandAndGraveyard(
        state,
        effect,
        targets,
        controllerId,
    ...
    if (
      effect.type === "PutRemainderOnBottomRandom" &&
    ...
    return this.resolveSingleTargetMove(
      state,
      effect,
      finalTargetIds,
      log,
  ...
  private static resolveInteractiveMovementSelection(
    ...
    if (targetDef.type === TargetType.CardInHand) pool = player.hand;
      ...
      pool = state.battlefield.filter((o) => o.controllerId === controllerId);
    else return;
    ...
    const resolvedMin =
      targetDef.minCount !== undefined
        ? EffectProcessor.resolveAmount(state, targetDef.minCount, context)
        : EffectProcessor.resolveAmount(state, targetDef.count || 1, context);
  ...
  private static resolveDrawCards(
    ...
    const { EffectProcessor } = require("../EffectProcessor");
    const playerIds = targets.filter(
    ...
    playerIds.forEach((pid) => {
      const amount = EffectProcessor.resolveAmount(
        state,
        effect.amount,
        context,
        [pid],
      );
      this.resolveLibraryTopMoves(
        state,
        {
          ...effect,
          selectionType: "TopN",
  ...
  private static resolveRevealUntilCondition(
    state: GameState,
    effect: EffectDefinition,
    log: (m: string) => void,
    context: ResolutionContext,
  ...
  private static resolveExchangeHandAndGraveyard(
    state: GameState,
    effect: EffectDefinition,
    targets: string[],
    controllerId: PlayerId,
  ...
  private static resolveMoveTargets(
    state: GameState,
    effect: EffectDefinition,
    targetIds: string[],
    log: (m: string) => void,
  ...
  private static resolveLibraryTopMoves(
    state: GameState,
    effect: EffectDefinition,
    controllerId: PlayerId,
    log: (m: string) => void,
    ...
    if (effect.type === EffectType.LookAtTopAndPick) {
      state.pendingAction = ChoiceGenerator.createCardChoice(state, cards, {
        ...
        minChoices:
          effect.selectionType === "AnyNumber" || effect.amount === "ANY"
            ? 0
            : 1,
        maxChoices:
          effect.selectionType === "AnyNumber" ||
          (effect as any).amount === "ANY"
            ? cards.length
            : (effect.amount as number) || 1,
        actionType:
          effect.optional || effect.selectionType === "AnyNumber"
            ? ActionType.OptionalAction
            : ActionType.ResolutionChoice,
          ...
          return [
            {
              ...
              libraryPosition:
                effect.remainderPosition || effect.libraryPosition || "bottom",
            },
          ];
        },
        stackObj: stackObject,
  ...
  private static resolveLibrarySearch(
    ...
    if (validCandidates.length === 0) {
      log(
        `[INFO] MoveEffectHandler: No valid objects in registry for "${effect.label || "Search"}". Auto-skipping search.`,
      );
      if (effect.shuffle && context && context.effects) {
    ...
    state.pendingAction = ChoiceGenerator.createCardChoice(state, pool, {
      ...
      filterSelectable: true,
      minChoices:
        effect.selectionType === "AnyNumber" || effect.amount === "ANY" ? 0 : 1,
      maxChoices:
        effect.selectionType === "AnyNumber" || effect.amount === "ANY"
          ? pool.length
          : (effect.amount as number) || 1,
      ...
      onSelected: (c: GameObject) => {
        ...
        if (log)
          log(
            `[DEBUG] resolveLibrarySearch.onSelected: Selected ${c.definition.name}. Zone: ${zone}`,
          );
  ...
  private static resolveMassMove(
    state: GameState,
    effect: EffectDefinition,
    targetIds: string[],
    context: ResolutionContext,
      ...
      if (player) {
        let pool = (sources as Zone[]).flatMap((z: Zone) => {
          ...
          if (z === Zone.Battlefield)
            return state.battlefield.filter((o) => o.controllerId === tid);
          return [];
        });
  ...
  private static resolveSingleTargetMove(
    ...
    idsToMove.forEach((tid) => {
      ...
      if (
        (effect.reveal || (effect as any).revealed) &&
        zone !== Zone.Battlefield
      ) {
        obj.isRevealed = true;
      }
