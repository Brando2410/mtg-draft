...
export class EffectProcessor {
    ...
    private static resolveInteractiveEffectSelection(state: GameState, effect: EffectDefinition, sourceId: string, controllerId: PlayerId, log: (m: string) => void, stackObject?: any, parentContext?: any) {
        ...
        const getRestrictions = (td: any) => {
            ...
            if (typeStr && !['ANY', 'CARD', 'PLAYER', 'OPPONENT', 'ANY_TARGET', 'CARD_IN_GRAVEYARD', 'CARD_IN_HAND', 'CARD_IN_LIBRARY', 'SELF', 'PERMANENT', 'SPELL'].includes(typeStr)) {
                res.push(typeStr);
            }
            return res;
        };

        const searchRestrictions = [...(effect.restrictions || []), ...getRestrictions(targetDef)];
