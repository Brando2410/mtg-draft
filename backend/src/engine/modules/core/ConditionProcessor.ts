import {
    ConditionContext, ConditionType, GameState,
    PlayerId, TargetingContext, TriggerEvent,
    Zone
...
export class ConditionProcessor {
  ...
  public static matchesCondition(
    ...
    const { sourceId, controllerId, event, stackObject } = context;
    ...
    if (typeof condition === "function") {
      return (condition as any)(state, event, context);
    }
    ...
    if (trimmedCondition.includes(":")) {
      ...
      switch (type) {
        ...
        case "OPPONENT_HAS_LIFE_LE":
          ...
          const opponent = Object.keys(state.players).find(
            (pid) => pid !== controllerId,
          );
          return opponent
            ? (state.players[opponent as PlayerId]?.life || 0) <= oppLife
            : false;
        case "EVENT_OBJECT_MATCHES": {
          const obj =
            ...
            event?.data?.object ||
            event?.data?.card ||
            (event as any)?.gameObject ||
            event?.object ||
            state.battlefield.find((o: any) => o.id === event?.sourceId);
          if (!obj) return false;
        ...
        case "TARGET_2_MATCHES":
          ...
          const targetId =
            event?.payload?.targetIds?.[targetIdx] ||
            event?.payload?.targetId ||
            (event as any)?.targetIds?.[targetIdx] ||
            (event as any)?.targets?.[targetIdx] ||
        ...
        case "TARGET_1_COUNTERS_P1P1": {
          const tId =
            (event as any)?.targetIds?.[0] || (event as any)?.targets?.[0];
        ...
        case "EVENT_MANA_VALUE_GE": {
          ...
          return (
            ManaProcessor.getManaValue(obj.definition.manaCost) >= threshold
          );
        }
        case "DRAWN_CARDS_GE":
          const threshold = parseInt(restrictions[0]);
          return (
            (state.turnState.cardsDrawnThisTurn[controllerId] || 0) >= threshold
          );
        case "X_EQUALS": {
        ...
        case "SPENT_MANA_LE": {
          const threshold = parseInt(restrictions[0]);
          const spent =
            event?.payload?.card?.data?.paidManaValue ||
            (event as any)?.data?.card?.paidManaValue ||
            (event as any)?.eventData?.spent ||
            (event as any)?.data?.spentMana ||
            (stackObject as any)?.data?.paidManaValue ||
            0;
        ...
        case "CONVERGE_GE": {
          ...
          const converge =
            obj?.convergeAmount ||
            (event as any)?.convergeAmount ||
            (event as any)?.data?.convergeAmount ||
            0;
        ...
        case "TOTAL_TOUGHNESS_GE": {
          ...
          const total = state.battlefield
            ...
            .reduce(
              (sum, obj) =>
                sum + LayerProcessor.getEffectiveStats(obj, state).toughness,
              0,
            );
        ...
        case "GRAVEYARD_COUNT_GE": {
          const threshold = parseInt(restrictions[0]);
          return (
            (state.players[controllerId]?.graveyard.length || 0) >= threshold
          );
        }
        case "HAND_COUNT_GE": {
          const threshold = parseInt(restrictions[0]);
        ...
        case "GRAVEYARD_CREATURE_COUNT_GE": {
          ...
          return (
            (state.players[controllerId]?.graveyard.filter((c) =>
        ...
        case "CREATURES_DIED_COUNT_GE": {
          const threshold = parseInt(restrictions[0]);
          return (
            (state.turnState.creaturesDiedThisTurn.length || 0) >= threshold
          );
        }
        case "EVENT_COUNTER_TYPE_MATCHES": {
          const expectedType =
            restrictions[0] === "p1p1" ? "+1/+1" : restrictions[0];
          const actualType =
            (event as any)?.counterType || (event as any)?.data?.counterType;
          const normalizedActualType =
            actualType === "p1p1" ? "+1/+1" : actualType;
    ...
    switch (trimmedCondition.toUpperCase()) {
      ...
      case "TARGETS_PERMANENT": {
        const targets =
          (event as any)?.targets ||
          (event as any)?.data?.targets ||
          (event as any)?.targetIds ||
          [];
        ...
        return targets.some((tid: string) => {
          ...
          return (
            ...
            [
              ...
              obj.definition.types.some((ot: string) => ot.toLowerCase() === t),
            )
          );
        });
      }
      ...
      case "CREATURE_DIED_UNDER_YOUR_CONTROL_THIS_TURN":
        return state.turnState.creaturesDiedThisTurn.some(
          (c) => c.controllerId === controllerId,
        );
      case "TARGET_1_IS_PREPARED": {
        const targetId =
          (event as any)?.targetIds?.[0] ||
          (event as any)?.targets?.[0] ||
          (event as any)?.targetId;
      ...
      case "HAS_COUNTERS": {
        const obj =
          ...
          event?.data?.object;
        return obj
          ? Object.values(obj.counters || {}).some((v) => (v as number) > 0)
          : false;
      }
      ...
      case "PUT_COUNTER_ON_SELF_THIS_TURN":
        return (
          state.turnState.countersAddedThisTurnIds?.includes(sourceId) || false
        );
      case "NOT_CAST_FROM_HAND": {
        const zone =
          (event as any).sourceZone ||
          (event as any).lastNonStackZone ||
          event?.data?.sourceZone ||
          event?.card?.lastNonStackZone;
      ...
      case "EVENT_OBJECT_CONTROLLER_IS_YOU": {
        const eObj =
          ...
          (event as any)?.gameObject ||
          event?.object ||
          state.battlefield.find(
            (o: any) => o.id === (event?.sourceId || (event as any)?.sourceId),
          );
      ...
      case "TRIGGER_TARGET_IS_SELF":
        return (
          event?.targetId === sourceId || event?.data?.targetId === sourceId
        );
      ...
      case "HAS_CREATURE_POWER_4_PLUS":
        return state.battlefield.some(
          (obj) =>
            ...
            obj.definition.types.some((t) => t.toLowerCase() === "creature") &&
            (Number(obj.definition.power || 0) >= 4 ||
              (obj.effectiveStats?.power || 0) >= 4),
        );
      case "SOURCE_IS_SELF":
      case "OBJECT_IS_SELF":
      case "EVENT_SOURCE_IS_SELF":
      ...
      case "SPENT_MANA_GT_POWER_OR_TOUGHNESS": {
        ...
        if (spent > 0)
          process.stdout.write(
            `[DEBUG] Increment check for ${obj.definition.name}: spent ${spent} > power ${stats.power} (base ${basePower}) or toughness ${stats.toughness} (base ${baseToughness})? ${met}\n`,
          );
        return met;
      }
      case "GRAVEYARD_CREATURE_COUNT_GE_3": {
        const creatures = state.players[controllerId].graveyard.filter((c) =>
      ...
      case "TOP_CARD_IS_GOBLIN": {
        ...
        return (
          topCard.definition.subtypes?.some(
            (s: string) => s.toLowerCase() === "goblin",
          ) || false
        );
      }
      case "HAS_INSTANT_AND_SORCERY_IN_GY": {
        const gy = state.players[controllerId]?.graveyard || [];
        const hasInstant = gy.some((c) =>
          c.definition.types?.some((t) => t.toLowerCase() === "instant"),
        );
        const hasSorcery = gy.some((c) =>
          c.definition.types?.some((t) => t.toLowerCase() === "sorcery"),
        );
      ...
      case "TARGET_IS_INSTANT_OR_SORCERY": {
        ...
        if (!tId) return false;
        const targetObj =
          state.stack.find((s) => s.id === tId)?.card ||
          state.battlefield.find((o) => o.id === tId);
        if (!targetObj) return false;
        const types = targetObj.definition.types.map((t: string) =>
          t.toLowerCase(),
        );
      ...
      case "CAST_INSTANT_SORCERY_THIS_TURN":
        return (
          state.turnState.instantOrSorceryCastThisTurn[controllerId] || false
        );
      ...
      case "TARGET_2_EXISTS": {
        return !!(
          (event as any)?.targetIds?.[1] || (event as any)?.targets?.[1]
        );
      }
      case "TARGET_3_EXISTS": {
        return !!(
          (event as any)?.targetIds?.[2] || (event as any)?.targets?.[2]
        );
      }
      case "OPPONENT_HAS_MORE_CARDS": {
        ...
        return Object.values(state.players).some(
          (p) => p.id !== controllerId && p.hand.length > player.hand.length,
        );
      }
      case "OPPONENT_CONTROLS_MORE_CREATURES": {
        ...
        const oppCreatures = state.battlefield.filter(
          (o) =>
      ...
      case "OPPONENT_ATTACKS_YOUR_PLANESWALKER": {
        ...
        return attackers.some((a) => {
          ...
          return (
            ...
            defender.definition.types.some(
              (t) => t.toLowerCase() === "planeswalker",
            )
          );
        });
      ...
      case "DISCARDEDCREATURE":
        ...
        return discIds.some((id) => {
          const obj = state.players[controllerId]?.graveyard.find(
            (c) => c.id === id,
          );
          return (
            obj &&
      ...
      case "SELFATTACKING":
        return (
          state.combat?.attackers.some((a) => a.attackerId === sourceId) ||
          false
        );
      case "NOTLAND":
      case "NON_LAND":
        ...
        return (
          (eObj &&
            !eObj.definition.types.some(
              (t: string) => t.toLowerCase() === "land",
            )) ||
          false
        );
      case "XIS1":
        return (event as any)?.xValue === 1;
      case "XIS2":
        return (event as any)?.xValue === 2;
      case "XIS3":
        return (event as any)?.xValue === 3;
      case "XIS4ORMORE":
        return ((event as any)?.xValue || 0) >= 4;
      case "YOUGAINEDLIFE":
      ...
      case "TARGETISCREATUREYOUCONTROLANDCOUNTERISP1P1": {
        ...
        return !!(
          obj &&
          obj.controllerId === controllerId &&
          (counterType === "+1/+1" || counterType === "p1p1")
        );
      }
      ...
      case "OPPONENT_HAS_MORE_CARDS_IN_HAND":
        ...
        return Object.values(state.players).some(
          (p) => p.id !== controllerId && p.hand.length > myHand,
        );
