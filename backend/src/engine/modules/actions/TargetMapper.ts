    ...
    public static resolveTargetMapping(
        ...
        switch (mapping) {
            ...
            case 'PARENT_CONTEXT_EXILED_IDS_OWNERS': {
                ...
                const owners = ids.map((id: string) => TargetValidator.findObjectInAnyZone(state, id)?.ownerId).filter(Boolean) as string[];
            ...
            case 'MATCHING_PERMANENTS_YOU_CONTROL':
                ...
                return state.battlefield
                    .filter(o => o.controllerId === controllerId && TargetValidator.matchesRestrictions(state, o, effect.restrictions, controllerId, sourceId, undefined, stackData))
                    .map(o => o.id);
            ...
            case 'ALL_MATCHING_PERMANENTS':
                if (!effect?.restrictions) return [];
                return state.battlefield
                    .filter(o => TargetValidator.matchesRestrictions(state, o, effect.restrictions, controllerId, sourceId, undefined, stackData))
                    .map(o => o.id);
            ...
            case 'MATCHING_CARDS': {
                ...
                return pool.filter(tid => {
                    ...
                    return obj && TargetValidator.matchesRestrictions(state, obj, effect.restrictions, controllerId, sourceId);
                });
            }
