...
import { TargetingContext } from "@shared/types/effects";
...
export function gameObjectRestriction(
    evaluator: (state: GameState, obj: GameObject, restriction: string, context: TargetingContext) => boolean
): IRestrictionHandler {
    return {
        matches(state: GameState, targetObj: Targetable, restriction: string, context: TargetingContext) {
...
export function playerRestriction(
    evaluator: (state: GameState, player: PlayerState, restriction: string, context: TargetingContext) => boolean
): IRestrictionHandler {
    return {
        matches(state: GameState, targetObj: Targetable, restriction: string, context: TargetingContext) {
...
export function stackObjectRestriction(
    evaluator: (state: GameState, obj: StackObject, restriction: string, context: TargetingContext) => boolean
): IRestrictionHandler {
    return {
        matches(state: GameState, targetObj: Targetable, restriction: string, context: TargetingContext) {
