...
export const GameCard = memo(({
  ...
  const headerFontSize = (() => {
    ...
    const base = len > 25 ? 0.8 : len > 18 ? 1.0 : 1.2;
    const variantScale = {
      hand: 1.8,
      stack: 1.1,
      small: 1.0,
    ...
    return `calc(var(--u) * ${base * variantScale} * ${contextScale})`;
  })();
  ...
  return (
    ...
    >
      ...
      {variant === 'battlefield' && !hideHeader && (
        <div
          style={{ ...(colors.length > 1 ? { background: (borderStyle as any).headerBackground } : {}), height: headerHeight }}
        ...
        `}>
          <h3
            ...
            className="font-black tracking-tighter truncate flex-1 min-w-0 text-left leading-none"
          >
        ...
        <div className={`${(variant === 'battlefield' || variant === 'zoom') ? 'relative flex-1' : 'absolute inset-0'} overflow-hidden ${dimensions.rounded}`}>
          ...
          {variant !== 'zoom' && (
            <div className={`absolute inset-0 w-full h-full overflow-hidden z-0 bg-slate-900`}>
              ...
              {variant !== 'battlefield' && (
                <>
                  {/* Header shroud to hide original printed text - Scales with --u to ensure coverage */}
          ...
          {variant !== 'zoom' && (
            <div className={`absolute inset-0 flex flex-col justify-end pointer-events-none z-20 
              ...
              {variant !== 'battlefield' && !hideHeader && (
                <div
                  style={{ ...(colors.length > 1 ? { background: (borderStyle as any).headerBackground } : {}), height: headerHeight }}
                  className={`absolute top-0 inset-x-0 flex items-center justify-between gap-1 px-[calc(var(--u)*0.8)] shadow-sm border-b
                          ${headerClass}
                      `}>
                  <h3
                    style={{ fontSize: headerFontSize }}
                    className="font-black leading-tight tracking-tighter drop-shadow-md truncate flex-1 min-w-0"
                  >
                    {formatName(displayName)}
                  </h3>
                  {variant !== 'tiny' && (
