        ...
        {!minimized && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-[#0b0f1a]/95 border border-white/10 p-5 rounded-[2.5rem] shadow-[0_40px_100px_rgba(0,0,0,0.9)] max-w-6xl w-full flex flex-row items-stretch gap-0 text-center relative overflow-hidden backdrop-blur-2xl"
            >
              {/* SOURCE CARD SECTION (Left) */}
              {sourceObject && (
                <div className="hidden lg:flex flex-col items-center justify-center p-14 bg-white/[0.04] border-r border-white/5 relative group/source min-w-[540px]">
                   ...
                   <div className="flex flex-col items-center gap-12">
                     <div className="flex flex-col items-center gap-2">
                        <p className="text-[11px] font-black uppercase tracking-[0.8em] text-indigo-400 italic drop-shadow-md">Trigger Source</p>
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-6 py-10 relative min-h-0 bg-black/5">
                  {/* BACKGROUND DECOR */}
                  ...
                  <div className="flex flex-col gap-1 mb-4 select-none">
                    <h3 className="text-5xl font-black italic uppercase tracking-tighter text-white drop-shadow-[0_0_30px_rgba(255,255,255,0.4)]">
                                          ...
                                          <div className="w-full aspect-[4/3] rounded-3xl border border-white/10 bg-white/5 flex flex-col items-center justify-center p-4 relative overflow-x-auto custom-scrollbar shadow-2xl">
                                              <Reorder.Group axis="x" values={scryState.graveyard} onReorder={(vals) => setScryState(p => ({ ...p, graveyard: vals }))} className="flex flex-row justify-center items-center w-full gap-4 px-4">
                                                  {scryState.graveyard.map((card: any) => (
                                                      <Reorder.Item key={card.id} value={card} className="relative scale-[0.8] cursor-grab active:cursor-grabbing shrink-0" drag="y" whileDrag={{ pointerEvents: 'none', zIndex: 100, scale: 0.6 }}
                          ...
                          <div className="flex flex-col items-center w-full px-12 py-4">
                              ...
                              <Reorder.Group axis="x" values={orderedTriggers} onReorder={setOrderedTriggers} className="flex flex-row justify-center gap-10 w-full py-4">
                                  {orderedTriggers.map((trigger) => (
                                      <Reorder.Item key={trigger.id} value={trigger} className="relative group/trigger cursor-grab active:cursor-grabbing">
                      ...
                      {!isOrderTriggers && !isScrySurveil && cardChoices.length > 0 && (
                          <div className="flex flex-wrap justify-center gap-8 p-6">
                              {cardChoices.map((choice: any) => {
                                  ...
                                  return (
                                      <motion.div key={choice.cardData.id} className={`relative cursor-pointer transition-all hover:translate-y-[-5px] ${isSelected ? 'z-20 scale-105' : 'z-10'}`} onClick={() => handleChoiceClick(originalIdx)}>
                      ...
                      {!isOrderTriggers && !isScrySurveil && buttonChoices.length > 0 && (
                          <div className="flex flex-col gap-5 w-full max-w-2xl mx-auto py-4">
                              {buttonChoices.map((choice: any) => {
                                  ...
                                  const isSelected = selectedIndices.includes(originalIdx);
                                  return (
                                      <button key={originalIdx} onClick={() => handleChoiceClick(originalIdx)} className={`w-full p-8 rounded-[2rem] border text-lg font-black uppercase italic tracking-[0.3em] transition-all hover:translate-y-[-4px] active:scale-[0.98] shadow-2xl ${isSelected ? 'bg-gradient-to-r from-indigo-600 to-indigo-700 border-yellow-400 text-white ring-8 ring-yellow-400/10' : 'bg-slate-800/40 hover:bg-slate-800/80 border-white/5 text-slate-400 hover:text-white'}`}>
                                          {choice.label}
                                      </button>
                                  );
                              })}
