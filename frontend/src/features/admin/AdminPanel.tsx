...
export const AdminPanel = ({ onClose }: AdminPanelProps) => {
   ...
   const destroyRoom = (roomId: string) => {
      if (confirm(`Are you sure you want to close this lobby ${roomId}?`)) {
                           ...
                           <div className="flex items-start justify-between mb-6">
                              <div>
                                 <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest ${room.status === 'drafting' ? 'bg-amber-500/20 text-amber-500' : 'bg-emerald-500/20 text-emerald-500'
                                       }`}>
                                       {room.status}
                                    </span>
                                 </div>
