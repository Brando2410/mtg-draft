...
export const Collection = ({ onBack, onSelectCube, onSelectDeck, onCreateNewCube, onCreateNewDeck }: CollectionProps) => {
  ...
  return (
    <div className="w-full h-full max-w-[1600px] mx-auto p-[clamp(1rem,4vw,3rem)] flex flex-col animate-in fade-in slide-in-from-bottom-6 duration-1000 selection:bg-indigo-500/30 overflow-hidden">

      {/* HEADER SECTION - Fluid proportions */}
      <div className="flex flex-col lg:flex-row items-center justify-between py-[clamp(1rem,2vh,2rem)] border-b border-white/5 gap-[clamp(1.5rem,2vw,3rem)] shrink-0 mb-8">
        <div className="flex items-center gap-[clamp(1rem,2vw,2rem)] w-full lg:w-auto">
          <button
            onClick={onBack}
            className="p-[clamp(0.75rem,2vw,1.25rem)] bg-slate-900/60 backdrop-blur-xl hover:bg-indigo-600 text-slate-500 hover:text-white rounded-[clamp(1rem,2vw,1.5rem)] border border-white/10 transition-all shadow-2xl group active:scale-95 shrink-0"
          >
            <Home className="w-[clamp(1rem,2vw,1.5rem)] h-[clamp(1rem,2vw,1.5rem)] group-hover:scale-110 transition-transform" />
          </button>
          <div className="space-y-1">
            <h2 className="text-[clamp(1.5rem,4vw,3.5rem)] font-black text-white uppercase tracking-tighter italic leading-none">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-[clamp(1rem,2vw,2rem)] w-full lg:max-w-3xl">
          ...
          <div className="flex bg-slate-950/80 p-1 rounded-xl border border-white/5 w-full sm:w-auto shadow-inner shrink-0">
            <button
              onClick={() => setActiveTab('cube')}
              className={`flex-1 sm:flex-none flex items-center justify-center gap-3 px-6 py-2.5 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'cube' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30 translate-y-[-1px]' : 'text-slate-500 hover:text-slate-300'}`}
            >
              <LayoutGrid size={14} />
              Draft Cubes
            </button>
            <button
              ...
              className={`flex-1 sm:flex-none flex items-center justify-center gap-3 px-6 py-2.5 rounded-lg font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'deck' ? 'bg-violet-600 text-white shadow-lg shadow-violet-600/30 translate-y-[-1px]' : 'text-slate-500 hover:text-slate-300'}`}
            >
              <Sword size={14} />
              Decks
            </button>
          ...
          <div className="relative w-full group min-w-[200px]">
            ...
            <input
              ...
              className="w-full bg-slate-900/40 backdrop-blur-md border border-white/5 text-white pl-12 pr-6 py-2.5 rounded-xl outline-none focus:border-indigo-500/50 transition-all font-bold text-sm placeholder:text-slate-600"
            />
          </div>
        </div>
      </div>
